...
export async function ensureSmsSchema(){
  ...
  for(const [k,v] of Object.entries(SMS_DEFAULTS)){
    await sql`INSERT INTO service_settings(setting_key,setting_value) VALUES(${k},${v}) ON CONFLICT(setting_key) DO NOTHING`;
  }
}
...
export async function getSmsLogs(limit=50){
  ...
  return sql`SELECT * FROM sms_logs ORDER BY created_at DESC LIMIT ${n}`;
}

export function getIcodeConfigState(){
  return {
    tokenConfigured:Boolean(process.env.ICODE_TOKEN_KEY),
    callbackConfigured:Boolean(process.env.ICODE_CALLBACK_PHONE),
    host:process.env.ICODE_HOST||'211.172.232.124',
    port:Number(process.env.ICODE_PORT||9201)
  };
}
...
function buildPacket({key,tel,cb,msg,title=''}){const payload={key,tel,cb,msg:String(msg).slice(0,2000),title,date:'',charset:'utf-8'};const json=JSON.stringify(payload);const bytes=Buffer.byteLength(json,'utf8');if(bytes>9999)throw new Error('문자 데이터가 너무 깁니다.');return `06${String(bytes).padStart(4,'0')}${json}`}

export async function sendIcodeSms(phone,message,{title='한빛 상품권'}={}){
...
export async function sendAndLogSms(args){
  ...
  const message=withPrefix(args.message,s.sms_prefix);
  try{
    const r=await sendIcodeSms(args.phone,message);
    const status=r.success?'sent':r.pending?'pending':'failed';
    await logSms({...args,message,status,resultCode:r.code,response:r.response});
    return {...r,status};
  }catch(e){
    await logSms({...args,message,status:'failed',response:e?.message||String(e)});
    throw e;
  }
}
...
export async function sendNewOrderSms({orderId,orderNo,customerName,phone,requestedAmount,expectedAmount}){
  const s=await getSmsSettings();
  const tel=normalizePhone(phone);
  const vars={접수번호:orderNo,신청자:customerName,전화번호:tel,신청금액:`${Number(requestedAmount||0).toLocaleString()}원`,입금금액:`${Number(expectedAmount||0).toLocaleString()}원`};
  const out=[];
  if(s.sms_received_customer_enabled==='1'){
    try{out.push(await sendAndLogSms({orderId,recipientType:'customer',phone:tel,eventType:'received_customer',message:renderSmsTemplate(s.sms_template_received_customer,vars)}))}catch(e){console.error('customer sms failed',e)}
  }
  if(s.sms_received_admin_enabled==='1'&&normalizePhone(s.sms_admin_phone)){
    try{out.push(await sendAndLogSms({orderId,recipientType:'admin',phone:s.sms_admin_phone,eventType:'received_admin',message:renderSmsTemplate(s.sms_template_received_admin,vars)}))}catch(e){console.error('admin sms failed',e)}
  }
  return out;
}
...
export async function sendOrderEventSms(orderId,eventType){
  const s=await getSmsSettings();
  const sql=getDb();
  const rows=await sql`SELECT id,order_no,customer_name,phone_encrypted,requested_amount,expected_amount FROM orders WHERE id=${Number(orderId)} AND deleted_at IS NULL LIMIT 1`;
  if(!rows.length)return[];
  const o=rows[0],phone=decryptText(o.phone_encrypted);
  const vars={접수번호:o.order_no,신청자:o.customer_name,전화번호:phone,신청금액:`${Number(o.requested_amount||0).toLocaleString()}원`,입금금액:`${Number(o.expected_amount||0).toLocaleString()}원`};
  let template='',enabled='0',type='';
  if(eventType==='reviewing'){template=s.sms_template_reviewing;enabled=s.sms_reviewing_enabled;type='status_reviewing'}
  if(eventType==='paid'){template=s.sms_template_paid;enabled=s.sms_paid_enabled;type='status_paid'}
  if(eventType==='rejected'){template=s.sms_template_rejected;enabled=s.sms_rejected_enabled;type='status_rejected'}
  if(enabled!=='1'||!template)return[];
  try{return [await sendAndLogSms({orderId:Number(orderId),recipientType:'customer',phone,eventType:type,message:renderSmsTemplate(template,vars)})]}catch(e){console.error('status sms failed',e);return[]}
}
